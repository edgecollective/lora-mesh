...
int intervalGPS = 1000;

static const uint32_t GPSBaud = 9600;

    unsigned long nextGPS = millis() + intervalGPS + random(1000, 3000);

TinyGPSPlus gps;

int delaySec = 3; 
uint8_t nodeId = 2; // change this for each node
...
// listen every loop for incoming messages so we can relay them:



if (nextGPS <= millis()) { // time to make a GPS measurement and send it
...
while (Serial1.available() > 0)
  {
    ...
    gpsreadcount++;
     ...
     nextGPS = millis() + intervalGPS + random(1000, 5000);
